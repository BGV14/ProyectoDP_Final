...
public class DetalleMatriculaController {
    ...
    @PostMapping
    public ResponseEntity<DetalleMatricula> registrar(@RequestBody DetalleMatricula recurso) {
        DetalleMatricula guardado = service.guardar(recurso);
        return ResponseEntity.status(HttpStatus.CREATED).body(guardado);
    }

    @PutMapping("/{id}")
    public ResponseEntity<DetalleMatricula> actualizar(@PathVariable Long id, @RequestBody DetalleMatricula recurso) {
