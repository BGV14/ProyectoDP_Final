...
public class DistritoController {
    ...
    @PostMapping
    public ResponseEntity<Distrito> registrar(@RequestBody Distrito recurso) {
        Distrito guardado = service.guardar(recurso);
        return ResponseEntity.status(HttpStatus.CREATED).body(guardado);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Distrito> actualizar(@PathVariable Long id, @RequestBody Distrito recurso) {
