...
public class PersonaController {
    ...
    @PostMapping
    public ResponseEntity<Persona> registrar(@RequestBody Persona recurso) {
        Persona guardado = service.guardar(recurso);
        return ResponseEntity.status(HttpStatus.CREATED).body(guardado);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Persona> actualizar(@PathVariable Long id, @RequestBody Persona recurso) {
